...
v3 improvements over v2:
  1. PRODUCTION DETECTION: Lower infer_production threshold (1 prod + 0 retrieval now fires
     when scale/infra keywords present); added more serving/monitoring/deployment keywords.
  2. PRODUCT ENGINEERING BONUS: New _product_engineering_bonus() detects production ownership,
     search infra, recommendation infra, platform engineering from responsibilities+projects.
  3. CAREER PROGRESSION: New _career_progression_score() rewards increasing seniority signals
     (Staff/Lead/Principal/tech-lead growth); soft penalty for regression.
  4. DYNAMIC SKILL WEIGHTING: Core JD skills (retrieval, ranking, embeddings, vector DB,
     production ML, Python) weighted 1.5x vs supporting skills (LangChain, prompt eng,
     fine-tuning) within tier scoring.
  5. TIEBREAK THRESHOLD: Only fires when |score_a - score_b| < 0.002 (truly close).
  6. REASONING DIVERSITY: Action verbs rotate per candidate_id hash; specific tech names
     (FAISS, Pinecone, BM25 etc.) extracted and mentioned when available.
  7. VALIDATION PASS: Before CSV write, top-20 verified for production/retrieval evidence;
     contradictory reasoning auto-corrected.
  8. BEHAVIOURAL CALIBRATION: Technical weight protected — behav capped at 0.25 weight,
     notice/response penalties reduced so they don't override strong tech fit.
  9. Encoding fix retained: utf-8-sig -> utf-8 -> cp1252 -> latin-1.
  10. No hardcoded candidate IDs, ranks, scores, or company names.
"""
...
TODAY           = date.today()
# FIX 1: Model was "all-MiniLM-L6-v2" in code but "BAAI/bge-small-en-v1.5" in
# comments/pipeline docs. Corrected to match the intended bge-small model.
BIENCODER_MODEL = "BAAI/bge-small-en-v1.5"

# Tiebreak fires only when scores within this delta
...
# v3: Core JD skills — weighted 1.5x in tier-A scoring
...
# v3: Supporting skills — full tier credit but not boosted
...
SKILLS_TIER_B = frozenset({
    ...
    # v3: supporting skills moved to tier-B equivalent
    "langchain", "llamaindex", "llama index", "prompt engineering",
    "langsmith", "langgraph",
})
...
# v3: Expanded production keywords — more implicit serving/deployment signals
...
# Production retrieval/search/recommendation specific signals
...
# v3: Product engineering ownership signals (for bonus)
...
# v3: Seniority progression signals for career progression analysis
SENIORITY_SIGNALS = {
    "senior": 3,
    "staff": 5,
    "principal": 6,
...
# Academic/demo-only signals — triggers soft penalty
...
# Product-company inference signals (no hardcoded company names)
...
# v3: Specific tech names to surface in reasoning
NOTABLE_TECH = [
    "faiss", "pinecone", "qdrant", "weaviate", "milvus", "chroma",
...
# v3: Action verb pool for reasoning diversity — rotated per candidate hash
ACTION_VERBS = [
...
# Well-known product companies that should never be flagged as IT services,
# even if their job descriptions lack enough PRODUCT_CO_SIGNALS hits.
# No hardcoded scores or ranks — only company classification.
...
def _infer_production(desc_lower: str) -> bool:
    """
    v3: Lower threshold — fire on ANY of:
      - >=2 production keywords
      - >=1 production + >=1 retrieval signal
      - >=1 strong infra signal (k8s/docker/fastapi) + >=1 scale signal
    Avoids false negatives on production profiles that don't use the word 'production'.
    """
...
def _is_product_company_job(job: dict) -> bool:
    """Infer product-company from job content. No hardcoded scores or ranks."""
    co = job.get("company", "").lower()
    # Explicit services blocklist
    if any(s in co for s in SERVICES_COMPANIES):
        return False
    # Known product company allowlist — always product regardless of signal count
...
def _academic_penalty(full_text: str) -> float:
    """Soft penalty for academic/demo-only profiles. -0.03 to -0.10."""
...
def _product_engineering_bonus(full_text: str) -> float:
    """
    v3: Small bonus for candidates demonstrating production ownership,
    search/recommendation infra, platform engineering.
    Derived from responsibilities + projects, no hardcoded company names.
    Up to +0.08.
    """
...
def _career_progression_score(career: list[dict]) -> float:
    """
    v3: Reward candidates with increasing seniority signals.
    Looks at title seniority levels across jobs (sorted by recency).
    Returns a bonus (+0.05 max) for upward progression,
    0.0 for flat/unclear, and a mild penalty (-0.03) for clear regression.
    """
    ...
    # Sort by start_date descending (most recent first); fall back to list order
    ...
    # Most recent vs oldest: positive delta = progression
    delta = levels[0] - levels[-1]
    if delta >= 2:
        return 0.05   # clear upward progression
    if delta == 1:
        return 0.03   # mild progression
    if delta == 0:
        return 0.0    # flat
    # Negative delta = regression (e.g. went from senior to junior title)
    return -0.03


def _get_action_verb(candidate_id: str) -> str:
    """
    v3: Rotate action verbs based on candidate_id hash for reasoning diversity.
    Deterministic — same candidate always gets same verb.
    """
...
def _extract_notable_tech(full_text: str) -> list[str]:
    """
    v3: Extract specific notable technologies found in candidate's full text.
    Returns up to 3 tech names to surface in reasoning.
    """
    ...
    out: list[str] = []
    for t in found:
        key = t.split()[0]  # first word as dedup key
        if key not in seen:
            seen.add(key)
            out.append(t)
        if len(out) >= 3:
...
def _is_binary_garbage(text: str) -> bool:
    """Return True if decoded text is clearly binary (ZIP/docx/pdf magic bytes)."""
    # PK = ZIP/docx/xlsx, %PDF = PDF, \x00 NULs abundant in binary streams
    raw = text[:16]
    if raw.startswith("PK"):          # ZIP container — docx/xlsx/pptx
        return True
    if raw.startswith("%PDF"):        # raw PDF
        return True
    # High density of non-printable chars in first 256 bytes = binary
    sample = text[:256]
...
def _extract_docx(path: Path) -> str:
    """Extract plain text from a .docx (or any ZIP-based Office file)."""
    try:
        from docx import Document  # python-docx
        doc = Document(str(path))
        paragraphs = [p.text for p in doc.paragraphs if p.text.strip()]
        # Also grab table cells — JDs sometimes use tables for requirements
        ...
        print(f"      [WARN] python-docx failed ({e}); falling back to raw XML extraction", flush=True)
        # Fallback: pull word/document.xml directly from the ZIP
...
def load_jd(jd_path: str | None = None, candidates_path: str | None = None) -> str:
    """
    Load JD from .md/.txt/.docx/.doc.
    - Auto-detects .docx by magic bytes (PK header) even if extension is .md
    - Encoding cascade for plain-text: utf-8-sig -> utf-8 -> cp1252 -> latin-1
    - Rejects binary garbage (latin-1 silently decodes anything, including docx bytes)
    """
    if jd_path is None:
        search_dirs = []
        if candidates_path:
        ...
        for d in search_dirs:
            for name in (
                "job_description.md", "job_description.docx",
                "job_description.txt", "jd.md", "jd.docx",
                "jd.txt", "JD.md", "JD.docx",
            ):
                candidate = d / name
                if candidate.exists():
    ...
    # Peek at magic bytes to detect binary Office/ZIP format regardless of extension
    ...
    if is_zip_container:
        # .docx, .xlsx, .pptx — extract text from the ZIP/XML layer
        print(f"      Detected .docx/ZIP format for {p.name} — extracting text...", flush=True)
        text = _extract_docx(p)
        used_encoding = "docx"
    elif is_pdf:
        print(f"ERROR: {p.name} is a PDF. Convert to .docx or .md first.", flush=True)
        sys.exit(1)
    else:
        # Plain text — try encodings in order; skip if result looks binary
        for enc in ("utf-8-sig", "utf-8", "cp1252", "latin-1"):
            try:
                candidate_text = p.read_text(encoding=enc)
                if _is_binary_garbage(candidate_text):
                    # e.g. a .docx renamed to .md; handle as ZIP
                    print(
                        f"      [{enc}] decoded to binary garbage — "
                        f"treating as .docx ZIP...", flush=True
                    )
                    text = _extract_docx(p)
    ...
    if not text or not text.strip():
        print(f"ERROR: Could not extract text from {jd_path}.", flush=True)
        sys.exit(1)

    print(f"      Loaded: {p.name} ({len(text.split())} words) [format: {used_encoding}]", flush=True)
    preview = text[:200].replace("\n", " ").replace("\r", "")
    print(f"      JD preview    : {preview!r}", flush=True)
    return text
...
def build_candidate_text(c: dict) -> str:
    """Includes career, projects, achievements for richer semantic signal."""
    p   = c["profile"]
...
def rule_score(c: dict, jd: dict) -> RuleScores:
    """
    v3 changes vs v2:
    - Core JD skills weighted 1.5x within tier-A scoring
    - Supporting skills (LangChain etc.) get 0.6x tier-B credit
    - _career_progression_score() adds up to +0.05 for seniority growth
    - _product_engineering_bonus() adds up to +0.08 for ownership evidence
    - _infer_production() threshold lowered (infra+scale combo fires it)
    - Senior title progression detected via SENIOR_TITLE_RE
    - behavioural score weight protected at W_BEHAV max
    """
    p      = c["profile"]
    sig    = c.get("redrob_signals", {})
    career = c.get("career_history", [])
    ...
    for sk in skills:
        ...
        # v3: core JD skills get 1.5x multiplier within tier-A
        is_core = name in SKILLS_CORE_JD
        # v3: supporting skills (LangChain etc.) get 0.6x tier-B credit
    ...
    # Content-fit bonus for retrieval-adjacent titles
    ...
    # v3: career progression bonus
    progression_bonus = _career_progression_score(career)

    # v3: product engineering ownership bonus
    ...
    # Store signals for tie-breaking
    c["_v3_signals"] = {
        ...
        "full_text":           full_text,   # for reasoning tech extraction
    }
    ...
    # ── 4. Behavioural score ────────────────────────────────────────────────────
    # v3: reduced notice/response penalty so tech fit stays dominant
    behav = 0.0
    ...
    # v3: reduced notice penalty — don't over-penalise strong technical candidates
    behav += (
...
def tiebreak_key(item: tuple) -> tuple:
    """
    v3: Tiebreak fires on truly close scores (|delta| < TIEBREAK_DELTA).
    Priority:
      1. hybrid score (primary)
      2. prod_retrieval_hits (confirmed AI/search/retrieval systems)
      3. jd_exact_signal (NDCG/MRR/A-B test match)
      4. product_co_ratio (time at product companies)
      5. prod_hits_any (any production evidence)
      6. product engineering ownership bonus
      7. github activity
      8. recruiter response rate
      9. shorter notice period
    """
...
def _validate_top20(top100: list[tuple], strong_thresh: float) -> list[str]:
    """
    v3: Lightweight validation of top-20.
    Returns list of warning strings (printed to stderr); does not modify ranks.
    Checks:
      - Scores are strictly non-increasing
      - Top-20 candidates have prod_retrieval_hits or prod_hits_any > 0
      - No candidate with 0 production evidence appears in top 10
    """
    warnings: list[str] = []
    prev_score = 1.0
...
def _find_production_company(c: dict) -> tuple[str, bool]:
    """Scans career + projects using _infer_production(). Prefers product-company hits."""
    career = c.get("career_history", [])
    fallback_svc = ("", False)
...
def _find_ops_signals(c: dict) -> list[str]:
    """Scans career + projects + achievements for ops signals."""
    career = c.get("career_history", [])
    all_proj_text = " ".join(
        proj.get("description", "") + " " + proj.get("name", "")
        for proj in c.get("projects", [])
...
def build_reasoning(
    ...
    """
    v3 reasoning:
    - Action verbs rotate per candidate hash (not always 'shipped')
    - Specific tech names (FAISS, Pinecone, BM25 etc.) extracted and mentioned
    - No generic 'Strong AI skills' — replaced with concrete evidence
    - 'No confirmed production' suppressed for strong-tier with high skill/sem
    - Contradictory reasoning avoided by checking score before adding concern
    """
    p      = c["profile"]
    sig    = c.get("redrob_signals", {})
    skills = c.get("skills", [])
    ...
    # v3: action verb for diversity
    action_verb = _get_action_verb(cid)

    # v3: extract notable tech for specific mentions
    ...
    # v3: suppress "no production" for strong-tier with strong tech evidence
    ...
    if tier_label == "strong":
        ...
        # v3: use specific tech name in reasoning when available
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Redrob Intelligent Candidate Ranker v3")
    ...
    jd      = parse_jd(jd_text)
    # FIX 2: Removed duplicate print statements for YoE range and target cities
    ...
    print(f"\n[5/5] Hybrid scoring, validation and writing top 100...", flush=True)
    ...
    warnings = _validate_top20(top100, strong_thresh)
    if warnings:
        print(f"  [VALIDATION] {len(warnings)} warning(s):", flush=True)
        for w in warnings:
            print(f"    ! {w}", file=sys.stderr, flush=True)
    else:
        print("  [VALIDATION] Top-20 passed all checks.", flush=True)
    ...
    print(
        ...
        f"{'Sem':>5}  {'Rules':>5}  {'ProdR':>5}  {'Tier':>8}  Title",
        flush=True,
    )
    for rank_idx, (hybrid, sem_n, rs, c) in enumerate(top100[:preview_n]):
        ...
        print(
            ...
            f"{sem_n:.3f}  {rs.total:.3f}  "
            f"{sv.get('prod_retrieval_hits', 0):>5}  "
            f"{tier_label:>8}  "
